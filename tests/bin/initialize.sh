...
then
	...
	npm run env run tests-cli "wp core multisite-convert --title='WP NewRelic Multisite'"
	...
	npm run env run tests-cli "cp wp-content/plugins/wp-newrelic/tests/cypress/.htaccess .htaccess"
fi
